...
def test_run():
    ...
    with patch.object(objects.BaseAtom, 'load_definition',return_value = None):
        ec_profile = Create(parameters = {'ECProfile.name' : 'test_param','job_id':'test_job_id','flow_id':'test_flow_id','ECProfile.plugin' : 'test_plugin','ECProfile.k' : 'k','ECProfile.m' : 'm'})
        with patch.object(Crud,'create',return_value = {'request':'test'}):
            with patch.object(Crud,'sync_request_status',return_value = None):
                ret = ec_profile.run()
                assert ret is True
        ec_profile = Create(parameters = {'ECProfile.name' : 'test_param','job_id':'test_job_id','flow_id':'test_flow_id','ECProfile.directory' : 'directory','ECProfile.k' : 'k','ECProfile.m' : 'm'})
        with patch.object(Crud,'create',return_value = {'request':'test'}):
            with patch.object(Crud,'sync_request_status',return_value = None):
                ret = ec_profile.run()
                assert ret is True
